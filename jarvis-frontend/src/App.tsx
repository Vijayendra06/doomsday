...
import { executeWebAction, isWebActionRequest, planWebAction, WebAction } from "./webActions";
...
import jarvisBackgroundVideo from "./assets/Looping_futuristic_interface_ani…_1080p_202609011657.mp4";
  ...
  async function askJarvis(text: string) {
    ...
    if (isLocalActionRequest(text)) {
      ...
      try {
        ...
        if (localAction.requiresConfirmation) {
    ...
    if (isWebActionRequest(text)) {
      ...
      try {
        ...
        const result = await executeWebAction(webAction);
        const reply = result.message.replace("The website", webAction.label);
        setMessages((current) => [...current, { author: "jarvis", text: reply }]);
        setStatus("idle");
        void playSpeech(reply);
      } catch (reason) {
  ...
  return (
    <main className="app-shell">
      <video className="background-video" autoPlay muted loop playsInline preload="auto" aria-hidden="true">
        <source src={jarvisBackgroundVideo} type="video/mp4" />
      </video>
