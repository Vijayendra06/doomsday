...
import { isCapabilityQuery, isInformationalQuery } from "./webActions";

export type LocalAction = {
  kind: "open_local_app";
  appId: "calculator" | "notepad" | "file_explorer" | "vscode" | "whatsapp";
  label: string;
...
export function isLocalActionRequest(text: string): boolean {
  if (isCapabilityQuery(text) || isInformationalQuery(text)) return false;
